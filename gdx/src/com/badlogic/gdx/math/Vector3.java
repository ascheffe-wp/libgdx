...
public class Vector3 implements Serializable, Vector<Vector3> {
	...
	public Vector3 prj (final Matrix4 matrix) {
		final float l_mat[] = matrix.val;
		final Double l_w = 1f / (x * new Double(l_mat[Matrix4.M30]) + y * new Double(l_mat[Matrix4.M31]) + z * new Double(l_mat[Matrix4.M32]) + new Double(l_mat[Matrix4.M33]));
		return this.set((x * l_mat[Matrix4.M00] + y * l_mat[Matrix4.M01] + z * l_mat[Matrix4.M02] + l_mat[Matrix4.M03]) * l_w.floatValue(), (x
			* l_mat[Matrix4.M10] + y * l_mat[Matrix4.M11] + z * l_mat[Matrix4.M12] + l_mat[Matrix4.M13])
			* l_w.floatValue(), (x * l_mat[Matrix4.M20] + y * l_mat[Matrix4.M21] + z * l_mat[Matrix4.M22] + l_mat[Matrix4.M23]) * l_w.floatValue());
	}
